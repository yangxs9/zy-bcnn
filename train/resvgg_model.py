...
class resvgg:
    ...
    def fc_layers(self):
        ...
        with tf.variable_scope('fc-new') as scope:
            fc3w = tf.get_variable('W', [786432, 120], initializer=tf.contrib.layers.xavier_initializer(), trainable=True)
            #fc3b = tf.Variable(tf.constant(1.0, shape=[100], dtype=tf.float32), name='biases', trainable=True)
            fc3b = tf.get_variable("b", [120], initializer=tf.constant_initializer(0.1), trainable=True)
