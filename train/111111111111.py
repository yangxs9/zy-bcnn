...
if __name__ == '__main__':
    ...
    dataset_path = '/media/File2/zhouyang/pig_face-master/raw_data/h5/';
    model_path = '/media/File2/zhouyang/pig_face-master/VGG16/';
    ...
    test_data_file = [dataset_path + 'testresult0.h5'];
    val_data_file = [];
    ...
    #parameter for test
    target_path = '/media/File2/zhouyang/pig_face-master/raw_data/txt/testresult0.txt';
    images = []
    with open(target_path, 'r') as f:   
        for l in f.readlines():
            ...
            images.append(name)
    csvfile = file('b_cnn_1' + 'test' +'.csv', 'wb')
    ...
    test_loss = 0.0
    while(test_reader.have_next()):
        batch_test_x, batch_test_y = test_reader.next_batch();
        test_loss += sess.run(loss, feed_dict={imgs: batch_test_x, target: batch_test_y, keep_prob: 1.0})
        pred = sess.run(num_correct_preds, feed_dict = {imgs: batch_test_x, target: batch_test_y, keep_prob: 1.0})
        correct_test_count+=pred
    test_loss = test_loss/(1.0*test_reader.total_datanum);
    print("##############################")
    print("test Loss -->", test_loss)
    print("correct_test_count, total_test_count", correct_test_count, test_reader.total_datanum)
    print("test Data Accuracy -->", 100.0*correct_test_count/(1.0*test_reader.total_datanum))
    print("##############################")
